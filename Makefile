...
export CC=${TOOLCHAIN_PATH}/aarch64-buildroot-linux-gnu-gcc
export CXX=${TOOLCHAIN_PATH}/aarch64-buildroot-linux-gnu-g++
...
toy-be: | ; $(info $(M) building toy be…)
	cd toy-be && env GOARCH=arm64 CGO_ENABLED=1 GOOS=linux GOGC=off GO111MODULE=on  $(GO) build -ldflags '$(LDFLAGS)' -o toy-be
